...
I am choosing to represent the component using only the running totals needed for
linear regression: the number of points, sumX, sumY, sumXX, sumYY, and sumXY.

My reasoning is pretty straightforward -- very kernel method becomes easy
to implement and efficient. Looking at this from a runtime optimization context,
adding a point is O(1), reporting the size is O(1), and all of the enhanced methods
can compute the regression statistics directly from these totals without needing to
store or loop through every original data point again.

The convention is that the number of points must be nonnegative, and when the
model is empty, all sums must be 0. The correspondence is that these fields
represent some collection of data points whose total x-values, y-values,
squared values, and x\*y products match the stored sums.
